...
function SteamImportModal({ currentUser, onClose, onImportComplete, onSteamConnected }) {
  ...
  const doImport = async () => {
    ...
    if (done > 0) {
      const havePlayed = toImport.filter(g => (statusOverrides[g.appid] || "have_played") === "have_played").length;
      const wantToPlay = toImport.filter(g => (statusOverrides[g.appid] || "have_played") === "want_to_play").length;
