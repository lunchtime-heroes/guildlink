    ...
    <div style={{ maxWidth: 900, margin: "0 auto", padding: isMobile ? "60px 16px 80px" : "80px 20px 40px" }}>
      ...
      {activeTab === "games" && (
        <div>
          ...
          {/* Search to add — full-screen modal on mobile, inline on desktop */}
          {addingGame && isMobile && ReactDOM.createPortal(
            <div style={{ position: "fixed", inset: 0, zIndex: 3000, display: "flex", flexDirection: "column", background: C.bg }}>
              <div style={{ display: "flex", alignItems: "center", gap: 12, padding: "16px 16px 12px", borderBottom: "1px solid " + C.border, flexShrink: 0 }}>
                <div style={{ position: "relative", flex: 1 }}>
                  ...
                  {gameSearch.length > 0 && (
                    <button onClick={() => { setGameSearch(""); setGameSearchResults([]); }}
                      style={{ position: "absolute", right: 10, top: "50%", transform: "translateY(-50%)", background: "none", border: "none", color: C.textDim, fontSize: 18, cursor: "pointer", padding: "0 4px", lineHeight: 1 }}>
                      ×
                    </button>
                  )}
                </div>
                <button onClick={() => { setAddingGame(false); setGameSearch(""); setGameSearchResults([]); }}
                  style={{ background: "none", border: "none", color: C.textMuted, fontSize: 15, fontWeight: 600, cursor: "pointer", padding: "8px 4px", flexShrink: 0 }}>
                  Cancel
                </button>
              </div>
              <div style={{ flex: 1, overflowY: "auto", WebkitOverflowScrolling: "touch" }}>
                {gameSearch.length >= 2 && gameSearchResults.length === 0 && (
                  <div style={{ padding: 24, color: C.textDim, fontSize: 14, textAlign: "center" }}>No games found</div>
                )}
                {gameSearchResults.map(game => (
                  <div key={game.id || game.igdb_id} style={{ display: "flex", alignItems: "flex-start", gap: 12, padding: "12px 16px", borderBottom: "1px solid " + C.border }}>
                    {game.cover_url
                      ...
                      <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
                        {SHELF_COLUMNS.map(col => (
                          <button key={col.id} onClick={async () => {
                            ...
                            style={{ background: col.color + "22", border: "1px solid " + col.color + "55", borderRadius: 3, padding: "7px 10px", color: col.color, fontSize: 12, fontWeight: 700, cursor: "pointer" }}>
                            {"+ " + col.label}
                          </button>
                        ))}
                      </div>
              ...
              <div style={{ background: C.surface, border: "1px solid " + C.border, borderRadius: 4, padding: 16, marginBottom: 16, position: "relative", zIndex: 11 }}>
                <div style={{ position: "relative" }}>
                  <input
                    ...
                    placeholder="Search for any game..."
                    style={{ width: "100%", background: C.surfaceRaised, border: "1px solid " + C.border, borderRadius: 3, padding: "8px 36px 8px 12px", color: C.text, fontSize: 13, outline: "none", boxSizing: "border-box" }}
                  />
                  {gameSearch.length > 0 && (
                    <button onClick={() => { setGameSearch(""); setGameSearchResults([]); }}
