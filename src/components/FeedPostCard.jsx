      ...
      {showComments && (
        <div style={{ borderTop: "1px solid " + C.border, padding: "12px 16px" }}>
          {(liveComments || []).map(comment => {
            ...
            const author = isNPCComment ? comment.npcs : comment.profiles;
            const authorName = isNPCComment ? (author?.name || "NPC") : (author?.username || "Unknown");
            const authorHandle = author?.handle || "";
            const authorInitials = isNPCComment ? (author?.avatar_initials || "?") : (author?.avatar_initials || "?");
            ...
            const parentName = parentComment
              ? (parentComment.npc_id ? (parentComment.npcs?.name || "someone") : (parentComment.profiles?.username || "someone"))
              : null;
            const goToCommentAuthor = () => {
              if (isNPCComment && comment.npc_id) {
                setCurrentNPC(comment.npc_id);
                setActivePage("npc");
                window.history.pushState({ page: "npc", npcId: comment.npc_id }, "", "/npc/" + comment.npc_id);
              } else if (comment.user_id) {
                setCurrentPlayer(comment.user_id);
                setActivePage("player");
                window.history.pushState({ page: "player", playerId: comment.user_id }, "", "/player/" + (comment.profiles?.handle?.replace("@","") || comment.user_id));
              }
            };
            return (
              <div key={comment.id} style={{ display: "flex", gap: 10, marginBottom: 14 }}>
                <div style={{ flexShrink: 0, cursor: "pointer" }} onClick={goToCommentAuthor}>
                  <Avatar initials={authorInitials} size={32} isNPC={isNPCComment} founding={!isNPCComment && author?.is_founding} ring={!isNPCComment ? author?.active_ring : null} avatarConfig={!isNPCComment ? author?.avatar_config : null} />
                </div>
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 6, flexWrap: "wrap", marginBottom: 3 }}>
                    <span style={{ fontWeight: 700, fontSize: 13, color: isNPCComment ? C.gold : C.text, cursor: "pointer" }}
