...
function LinkPreviewCard({ preview, onExit }) {
  ...
  const displayDomain = preview.domain?.replace(/^www\./, "") || (() => { try { return new URL(preview.url).hostname.replace(/^www\./, ""); } catch { return preview.url; } })();
  ...
  return (
    <div onClick={e => { e.stopPropagation(); if (onExit) { onExit(preview.url); } else { window.open(preview.url, "_blank", "noopener,noreferrer"); } }}
      style={{ marginTop: 10, background: C.surfaceRaised, border: "1px solid " + C.border, borderRadius: 4, overflow: "hidden", display: "flex", cursor: "pointer", textDecoration: "none" }}
...
function ExitModal({ url, onClose }) {
  ...
  return (
    <div onClick={onClose} style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.7)", zIndex: 1000, display: "flex", alignItems: "center", justifyContent: "center", padding: 20 }}>
      <div onClick={e => e.stopPropagation()} style={{ background: C.surface, border: "1px solid " + C.border, borderRadius: 4, padding: 28, maxWidth: 400, width: "100%", textAlign: "center" }}>
        ...
        <div style={{ display: "flex", gap: 10, justifyContent: "center" }}>
          <button onClick={onClose} style={{ background: C.surfaceRaised, border: "1px solid " + C.border, borderRadius: 3, padding: "8px 20px", color: C.textMuted, fontSize: 13, cursor: "pointer" }}>Cancel</button>
          <button onClick={() => { window.open(url, "_blank", "noopener,noreferrer"); onClose(); }}
            style={{ background: C.accent, border: "none", borderRadius: 3, padding: "8px 20px", color: "#fff", fontSize: 13, fontWeight: 700, cursor: "pointer" }}>Continue →</button>
        </div>
      </div>
    </div>
  );
