    ...
    <nav style={{
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 10, marginLeft: 10 }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 3 }}>
          <span style={{ color: C.gold, fontSize: 10, opacity: 0.7, userSelect: "none", fontWeight: 600 }}>b0317-191</span>
...
function ChartsPage({ setActivePage, setCurrentGame, isMobile }) {
  ...
  const getWeekStarts = (count) => {
    const starts = [];
    const base = getWeekStart(); // already a YYYY-MM-DD string in Pacific time
  ...
  useEffect(() => {
    const load = async () => {
      ...
      const newSparklines = {};
      top10.forEach(g => { newSparklines[g.id] = buildSp(g.id, sparkEvents || [], allWeekStarts); });
      setSparklines(newSparklines);
    };
    load();
  }, [chartRange]);

  const buildSp = (gameId, events, weekStarts) => {
    const weekScores = {};
    weekStarts.forEach(w => { weekScores[w] = { score: 0, users: new Set() }; });
    events.filter(e => e.game_id === gameId || !e.game_id).forEach(e => {
    ...
    const ordered = weekStarts.slice().reverse();
  ...
  const loadSparkline = async (gameId) => {
    ...
    setSparklines(prev => ({ ...prev, [gameId]: buildSp(gameId, (events || []).map(e => ({ ...e, game_id: gameId })), weekStarts) }));
  ...
  // Sparkline always renders 9 slots: 8 weekly data points + 1 empty future slot
  // points must be exactly 9 values (8 scores oldest→newest, then 0 for future)
  // labels must be exactly 9 strings
  const Sparkline = ({ points, labels, color = C.accent }) => {
    ...
    // Only draw line/area through non-zero segments
    const segments = [];
    let cur = [];
    points.forEach((v, i) => {
      if (v > 0) { cur.push([i, v]); }
      ...
      <div style={{ marginTop: 8 }}>
        <svg width={w} height={h} style={{ display: "block" }}>
          ...
          {segments.map((seg, si) => (
            <g key={si}>
              <path
                d={`M ${xPos(seg[0][0])},${h - pad} ` + seg.map(([i, v]) => `L ${xPos(i)},${yPos(v)}`).join(" ") + ` L ${xPos(seg[seg.length-1][0])},${h - pad} Z`}
                fill={`url(#grad-${color.replace("#","")})`}
              />
              <polyline
                points={seg.map(([i, v]) => `${xPos(i)},${yPos(v)}`).join(" ")}
          ...
          {points.map((v, i) => v > 0 ? (
            <circle key={i} cx={xPos(i)} cy={yPos(v)}
              r={i === points.length - 2 ? 3.5 : 2}
              fill={color} opacity={i === points.length - 2 ? 1 : 0.6}
            />
          ) : null)}
        </svg>
        <div style={{ position: "relative", height: 14, marginTop: 2, width: w }}>
          {labels.map((l, i) => (
            <span key={i} style={{ position: "absolute", left: xPos(i), transform: "translateX(-50%)", color: i === slots - 1 ? "transparent" : C.textDim, fontSize: 9, whiteSpace: "nowrap" }}>{l}</span>
          ))}
        </div>
      </div>
    );
        ...
        {isExpanded && (
          <div style={{ padding: "4px 20px 18px", borderTop: "1px solid " + C.border, background: C.accentGlow }}>
            ...
            {isLoadingSp ? (
              ...
              <Sparkline points={sp} labels={spLabels} color={C.accent} />
            ) : (
    ...
    <div style={{ maxWidth: 760, margin: "0 auto", padding: isMobile ? "60px 12px 80px" : "80px 24px 40px" }}>
      ...
      <div style={{ display: "flex", gap: 6, marginBottom: 28 }}>
        {[{ id: "7d", label: "This Week" }, { id: "30d", label: "This Month" }].map(w => (
          <button key={w.id} onClick={() => { setChartRange(w.id); setExpandedOverall(null); setExpandedGenre({}); setExpandedGenreAll(new Set()); }}
            style={{ background: chartRange === w.id ? C.accentGlow : C.surface, border: "1px solid " + chartRange === w.id ? C.accentDim : C.border, borderRadius: 20, padding: "6px 16px", color: chartRange === w.id ? C.accentSoft : C.textMuted, fontSize: 13, fontWeight: chartRange === w.id ? 700 : 500, cursor: "pointer" }}>
            {w.label}
          </button>
        ))}
      </div>
        ...
        <>
          ...
          <div style={{ background: C.surface, border: "1px solid " + C.border, borderRadius: 16, marginBottom: 32, overflow: "hidden" }}>
            <div style={{ padding: "16px 20px 12px", borderBottom: "1px solid " + C.border, display: "flex", alignItems: "baseline", gap: 10 }}>
              ...
              <div style={{ color: C.textDim, fontSize: 12 }}>{windowLabel[chartRange]}</div>
            </div>
            {overall.map((entry, i) => (
...
export default function GuildLink() {
  ...
  return (
    <div style={{ minHeight: "100vh", background: C.bg, backgroundImage: C.bgPattern || "none", color: C.text }}>
      ...
      {activePage === "player" && <PlayerProfilePage userId={currentPlayer} setActivePage={setActivePage} setCurrentGame={setCurrentGame} setCurrentNPC={setCurrentNPC} setCurrentPlayer={setCurrentPlayer} isMobile={isMobile} currentUser={liveUser} isGuest={isGuest} onSignIn={openSignIn} setGameDefaultTab={setGameDefaultTab} />}
