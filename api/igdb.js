...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (query) {
      ...
      const r = await fetch("https://api.igdb.com/v4/games", { method: "POST", headers, body });
      const games = await r.json();
      // Filter out DLC, expansions, bundles, mods
      const categoryFiltered = (games || []).filter(g => ![1, 2, 3, 6].includes(g.category));
      // Keep games with cover art, follows > 0, or are a main game (category 0)
      const qualityFiltered = categoryFiltered.filter(g =>
        g.cover?.image_id || (g.follows || 0) > 0 || g.category === 0
      );
      // Sort: cover art first, then by follows
